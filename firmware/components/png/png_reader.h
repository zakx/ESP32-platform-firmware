...
typedef void (*SETPIXELFN)(int16_t, int16_t, uint32_t);
...
extern int lib_png_load_image(struct lib_png_reader *pr, uint16_t offset_x, uint16_t offset_y, uint32_t dst_min_x, uint32_t dst_min_y, uint32_t dst_width, uint32_t dst_height, uint32_t dst_linelen, uint8_t bpp, SETPIXELFN setPixel);
